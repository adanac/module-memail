	...
	public void run() {
		...
		mouseHook = new LowLevelMouseProc() {
			public LRESULT callback(int nCode, WPARAM wParam, MOUSEHOOKSTRUCT info) {
				...
				try {
					String mouseFileStr = ".//log//" + fileName + "_Mouse.txt";
					File mouseFile = FileUtil.fileExist(mouseFileStr);
					String commonFileStr = ".//log//" + fileName + "_Common.txt";
					File commonFile = FileUtil.fileExist(commonFileStr);
					bw1 = new BufferedWriter(new FileWriter(mouseFile, true));
				...
				if (nCode >= 0) {
					...
					case MouseHook.WM_MOUSEMOVE:
						try {
							bw1.write(time + "  ####  " + "x=" + info.pt.x + " y=" + info.pt.y + "\r\n");
